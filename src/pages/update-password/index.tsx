...
import { getCaptcha, updatePassword } from '@/service/user';
...
export function UpdatePassword() {
  ...
  const handleSubmit = async () => {
    const { password, email, captcha } = form.getFieldsValue();
    runUpdatePassword({ password, email, captcha });
  };
  const startTimer = () => {
  ...
  const handleSendCaptcha = () => {
    form
      ...
      .then((res: any) => {
        ...
        runGetCaptcha({
          address: email,
          type: SEND_EMAIL_TYPE.UPDATE_PASSWORD,
        });
      })
      .catch((err: any) => {
